...
if ($_GET['res'] == logoff) {
  ...
  $sql = "SELECT `username`, `acctstarttime`, `acctinputoctets`, `acctoutputoctets`
  FROM
  radacct as Table1,
  (
  SELECT max(acctstarttime) as LATEST
  FROM radacct
  WHERE username='$login_user'
  ) as Tabel2
  WHERE acctstarttime=`LATEST`";
  $result = mysqli_query($db, $sql);
  $row = mysqli_fetch_array($result, MYSQLI_ASSOC);
  echo "input " . $row['acctinputoctets'] . " bytes<br />";
  echo " output " . $row['acctoutputoctets'] . " bytes<br />";
  // ......
  print_footer();
  session_destroy();
}
...
if ($_GET['res'] == popup2) {
  ...
  print_bodyz();
  // ...
  // PRINT OUT DATA FLOW to USER
  # Connect to DATABASE!
  $db = mysqli_connect("localhost", "radius", "radpass", "radius");
  $login_user = $_SESSION['login_user'];
  print "hello user:" . $_SESSION['login_user'] . "!<br>";
  // ...
  $sql = "SELECT `username`, `acctstarttime`, `acctinputoctets`, `acctoutputoctets`
  FROM
  radacct as Table1,
...
function print_login_form(){
  ...
  print '<FORM name="form1" METHOD="get" action="' . $loginpath . '?">
		...
		<INPUT type="button" onclick="go_to_register()" value="Register Now!" />
        </center>
	  </form>';
}
